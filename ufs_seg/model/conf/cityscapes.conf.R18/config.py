...
C.log_dir = osp.abspath(osp.join(C.root_dir, 'log', 'cityscapes_conf_R18'))
...
C.dataset_path = "/semantic/datasets/Cityscapes"
C.img_root_folder = C.dataset_path
C.gt_root_folder = C.dataset_path
C.train_source = osp.join(C.dataset_path, "train.citys")
C.eval_source = osp.join(C.dataset_path, "test.citys")
# C.eval_source = osp.join(C.dataset_path, "visual_list.citys")
...
C.bn_eps = 1e-5
C.bn_momentum = 0.1
C.pretrained_model = "/semantic/model_zoo/resnet18_v1.pth"
...
C.eval_scale_array = [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2.0]  # multi scales: 0.5, 0.75, 1, 1.25, 1.5, 1.75
