...
C.log_dir = osp.abspath(osp.join(C.root_dir, 'log', 'voc_conf_R50'))
...
C.dataset_path = "/semantic/datasets/pascal_VOC_2012/VOC_aug"
C.img_root_folder = osp.join(C.dataset_path, "img")
C.gt_root_folder = osp.join(C.dataset_path, "gt")
C.train_source = osp.join(C.dataset_path, 'train_aug.txt')
# C.train_source = osp.join(C.dataset_path, 'train.txt')
C.eval_source = osp.join(C.dataset_path, 'val_aug.txt')
...
C.bn_eps = 1e-5
C.bn_momentum = 0.1
C.pretrained_model = "/semantic/model_zoo/resnet50_v1c.pth"
